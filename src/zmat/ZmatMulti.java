 ...
 * @author Xiaoxing
 */
public class ZmatMulti extends ZmatDual {
    ...
    public void processFile(String... s) {
        dp = new DataProcessor() {
            @Override
            public void processFile(String... s) {
                FileParser fp = new zmat.sessionparser.dualtaskparser.MultiParser();
                fp.parseFiles(s);
                days = fp.getDays();
                if (days.size() < 1) {
        ...
        for (Day d : days) {
            ...
            for (Session s : d.getSessions()) {
                ...
                for (Trial tt : s.getTrails()) {
                    zmat.sessionparser.dualtaskparser.MultiTrial t = (zmat.sessionparser.dualtaskparser.MultiTrial) tt;
                    ...
                    boolean trialDistracted = (distracted == 2) || ((distracted == 0) == (t.getDistractorResponse().isEmpty()));
                    ...
                    if (trialLightOn & trialDistracted) {
                        ...
                        for (EventType e : t.getDistractorResponse()) {
                            switch (e) {
                                case Hit:
                                    distractroHit++;
                                    break;
