 ...
 * @author Libra
 */
public class MultiParser extends zmat.dnms_session.FileParser {   //Dual task ODPA On Off Laser
    ...
    protected Queue<Session> processFile(File f) {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(f))) {
            ...
            ArrayList<EventType> distractorOdor = new ArrayList<>();
            boolean laserOn = false;
            EventType response;
            ArrayList<EventType> distractorResponse = new ArrayList<>();
            int lastLick = 0;
...
//                            System.out.println(""+evt[2]+", "+evt[3]);
                        if (evt[3] == 2 || evt[3] == 1) {
                            ...
                            if (firstOdor != EventType.unknown && secondOdor != EventType.unknown) {
                                currentTrials.offer(new MultiTrial(firstOdor, secondOdor, response, laserOn, licks, delayLength, odor2Start, distractorOdor, distractorResponse));
                            }
                            firstOdor = EventType.unknown;
                            secondOdor = EventType.unknown;
                            distractorOdor = new ArrayList<>();
                            distractorResponse = new ArrayList<>();
//                        licks = new ArrayList<>();
                            laserOn = false;
                        } else if (evt[3] == 3) {
                            distractorResponse.add(responses[evt[2] - 4]);
...
//                                licks = new ArrayList<>();
                    ...
                    case 64:
                    case 66:
                        distractorOdor.add((evt[2] == 66) ? EventType.OdorA : EventType.OdorB);
                        break;
                    case 65:
                        laserOn = (evt[3] == 1);
                        break;
